...
func _init(genome_id: int, initial_neurons: Dictionary, initial_links: Dictionary, parent1ID: int, parent2ID: int) -> void:
...
func generate_agent(parent_position: Vector2) -> Agent:
	...
	agent = Agent.new(new_network, is_leader_clone, parent_position, id)
	return agent
