...
func create_initial_population() -> Array:
	...
	for _initial_genome in Params.population_size:
		...
		curr_agents.append(new_genome.generate_agent(Vector2(0,0)))
	...
	for species in curr_species:
		...
		for spawn in species.num_to_spawn:
			...
			if (parent1) and (parent2):
				parent_position = parent1.body.global_position if Utils.random_f() < 0.5 else parent2.body.global_position
			elif parent1:
				parent_position = parent1.body.global_position
			elif parent2:
				parent_position = parent2.body.global_position
			else:
...
#			print("SPAWNING AT: ", mom.agent.body.global_position)
			curr_agents.append(baby.generate_agent(mom.agent.body.global_position))
			hybrids.append(baby)
		# go to next species
		species_index += 1 
