...
#	leader = alive_members[0]
	var i = 0
	while !leader.agent.body:
		leader = alive_members[i]
		i+=1
	return leader
...
#	print("CLEARED ", len(alive_members)-len(new_alive), "/", len(alive_members))
	alive_members = new_alive
...
func calculate_offspring_amount(total_avg_species_fitness, alive: int = 0) -> void:
	...
	if age != 0:
#		print("-------------------", total_avg_species_fitness)
		num_to_spawn = round((avg_fitness_adjusted / total_avg_species_fitness) * (Params.population_size-alive))
