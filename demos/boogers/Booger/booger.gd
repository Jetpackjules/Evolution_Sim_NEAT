...
var move_hunger = 0.0 # Set the amount of hunger consumed when the agent moves
var thirst = 0.0

var predator_chance = 0.0  # Chance of becoming a predator (0 to 1)
...
func _ready() -> void:
	...
	if randf() < predator_chance:
		carnivore = true
	
	if carnivore:
#		remove_from_group("prey")
		add_to_group("predator")
		$Sprite.texture = load("res://demos/boogers/Booger/Cannibal_Booger.png")
	else:
#		remove_from_group("predator")
		add_to_group("prey")
		$Sprite.texture = load("res://demos/boogers/Booger/booger.png")
...
func _physics_process(_delta) -> void:
	...
	"""
	# Show hunger:
#	$Label.text = str(stepify(-hunger+starvation_threshold, 0.1))
	$Label.text = str(stepify(-thirst+thirst_threshold, 0.1))
	...
	hunger += (_delta/10.0)*hunger_multiplier # Reduced passive hunger increase
#	thirst += (_delta/20.0)
...
func sense() -> Array:
	...
	for caster in raycasters:
		...
		if caster.is_colliding():
			...
			elif collided_object.is_in_group("water"):
				senses.append(2)
			elif collided_object.is_in_group("danger"):
				senses.append(-1)  # -1 represents danger
			elif collided_object.is_in_group("booger"):
				senses.append(3)
			else:
...
#	senses.append(-thirst+thirst_threshold)
	return senses
...
func get_fitness() -> float:
	...
	return food_score + water_score + life_score

func eat_agent(agent) -> void:
	agent.dead = true
	agent.emit_signal("death")
	food_score += agent.food_score * 0.5  # You can adjust the multiplier for how much food_score you gain
	hunger -= (-agent.hunger+starvation_threshold) * 0.5  # You can also adjust the multiplier for how much hunger is reduced

func on_body_entered(body) -> void:
	if body.is_in_group("prey") and (body.get_node("Sprite").scale.x <= (get_node("Sprite").scale.x)) and is_in_group("predator"):
		print("GODDDAMIT!!")
		eat_agent(body)
		print(body.position)
#		breakpoint
