...
onready var GameOverSplash = preload("res://demos/boogers/race_mode/GameOverSplash.tscn")
...
func setup_race(track_num: int, opponent_names: Array) -> void:
	...
	var track_path = "res://demos/boogers/tracks/track_%s/Track_%s.tscn" % [track_num, track_num]
	add_child(load(track_path).instance())
	# make a player car instance and adds it to the start
	player = load("res://demos/boogers/car/PlayerCar.tscn").instance()
...
class CarAgent:
	...
	func _init(opponent_name: String) -> void:
		...
		car = load("res://demos/boogers/car/Booger.tscn").instance()
