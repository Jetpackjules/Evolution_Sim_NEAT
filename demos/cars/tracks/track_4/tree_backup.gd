...
#	{"id": 2, "parent1_id": 1, "parent2_id": null},
#	{"id": 3, "parent1_id": 1, "parent2_id": null},
#	{"id": 4, "parent1_id": 2, "parent2_id": null},
#	{"id": 5, "parent1_id": 2, "parent2_id": null},
#	{"id": 6, "parent1_id": 3, "parent2_id": null},
#	{"id": 7, "parent1_id": 3, "parent2_id": null},
#	{"id": 8, "parent1_id": 4, "parent2_id": null},
#	{"id": 9, "parent1_id": 4, "parent2_id": null},
#	{"id": 10, "parent1_id": 5, "parent2_id": null},
#	{"id": 11, "parent1_id": 5, "parent2_id": null},
#	{"id": 12, "parent1_id": 6, "parent2_id": null},
#	{"id": 13, "parent1_id": 6, "parent2_id": null},
#	{"id": 14, "parent1_id": 7, "parent2_id": null},
#	{"id": 15, "parent1_id": 7, "parent2_id": null},
...
func calculate_node_positions(nodes, generations):
	var generation_offset_base = 150
	var angle_offset = PI*2

	for gen in generations:
		var siblings = generations[gen]
		var total_sibling_count = siblings.size()
		var angle_step = angle_offset / total_sibling_count
		var angle_start = -angle_offset / 2 + (gen % 2) * angle_step / 2


		for i in range(siblings.size()):
			var node = siblings[i]
			var angle = angle_start + i * angle_step
			var radius = pow(gen + 1, 1.3) * generation_offset_base  # Adjust the exponent (1.3) for different spacing
			var x_pos = radius * cos(angle)
			var y_pos = radius * sin(angle)
			node.position = Vector2(x_pos, y_pos) + rect_size / 2
			print(node.position)
	...
	for node in nodes.values():
		for connection in node.connections:
			if node.generation < connection.generation:
				...
				line.add_point(node.position + Vector2(25, 25))
				line.add_point(connection.position + Vector2(25, 25))
				add_child(line)
				...
				if node.father_id != null:
					...
					label.rect_position = (node.position + connection.position) / 2 + Vector2(0, -10)
					add_child(label)
					added_elements.append(label)
